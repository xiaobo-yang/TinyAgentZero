...
EXP_NAME=$ENV-$MODEL_NAME-PPO-${RUN_TIME}-rephrase-sft-shuffle-multi-val
git add .
...
python -m src.core.main_ppo \
    +env.name=$ENV \
    data.train_files=$DATA_PATH/deepscaler_train_shuffle.parquet \
    ...
    +actor_rollout_ref.actor.sft_weight=30 \
